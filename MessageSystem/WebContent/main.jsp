...
<html>
	...
	<%
		...
		//System.out.println("현재 로그인한 email 값 : "+email);
			...
			<div id="wrapper">
				...
				<!-- Header -->
					<header id="header" class="alt">
						<a href="index.html" class="logo"><strong>방문자 수</strong> <span><%=cnt %></span></a>
						<nav>				
								<%if(dto==null){%>
									<a href="#menu">로그인</a>
								<%}else{%>
										<!-- 로그인한 이메일이 'admin'이였을 때 아래 회원전체검색 메뉴가 출력되게하세요 -->
						...
						<ul class="actions vertical">
							<li><h5>회원가입</h5></li>
								<form action="JoinCon.do" method="post">
									...
									<br><p id="result"></p>
									<button type="button" id="emailCheck" onClick="emailCheck2()">중복체크</button>
									</li>
									<li><input type="password" name="pw"  placeholder="PW를 입력하세요" ></li>
				...
				function emailCheck2(){
					$.ajax({
						...
						success : function(res){
							$('p#result').append(res)
							// 기본적으로 out객체로 응답받는 데이터는 text/html
							if(res=='true'){
								$('p#result').html("중복된 아이디입니다.").css("color","red");
								//$('p#result').css("color","red")
							}else{
								$('p#result').html("사용가능한 아이디입니다.").css("color","green")
							}
						},
						error : function(){
							alert("요청 실패!")
